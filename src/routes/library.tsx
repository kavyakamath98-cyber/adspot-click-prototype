...
function LibraryPage() {
  ...
  const results = useMemo(() => {
    ...
    return creatives.filter(
      (c) => c.name.toLowerCase().includes(s) || c.tags.some((t) => t.toLowerCase().includes(s)),
    );
  }, [creatives, q]);
    ...
    <AppShell>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="relative w-full max-w-xs">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by name or tag" className="pl-9" />
        </div>
      </div>
      ...
      {results.length === 0 ? (
        <Card className="p-16 text-center">
          ...
          <h3 className="font-semibold">No creatives found</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            {q ? "Try a different search." : "Upload one from a new campaign."}
          </p>
        </Card>
      ) : (
